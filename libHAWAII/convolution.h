	...
	for(; out_ch < D_OUT->CH; out_ch++){
		for(; in_ch < D_IN->CH; in_ch++){
				...
				for(; f_row < PARA->KERNEL_H ; f_row++){
					...
					if(fp_temp){
						*(sub_FP_3+ ((f_row+1) & 0x1)) = length_out;
						...
						DMA0DA = (LEA_MEMORY + length_in + ks);
						DMA0SZ = ks;
						DMA0CTL |= DMAEN__ENABLE + DMAREQ + DMAIE;
						__bis_SR_register(GIE+LPM0_bits);
						...
						//set input offset
						lea_FIR_Params->coeffs = MSP_LEA_CONVERT_ADDRESS(LEA_MEMORY + length_in);
						lea_FIR_Params->output = MSP_LEA_CONVERT_ADDRESS(LEA_MEMORY + length_in + ks + 1 + length_out - fp_temp);
						lea_FIR_Params->vectorSize = fp_temp + (fp_temp & 0x1);

						LEAPMS0 = MSP_LEA_CONVERT_ADDRESS(LEA_MEMORY + f_row * D_IN->W + length_out - fp_temp);
						...
						DMA2SA = (LEA_MEMORY + length_in + ks + 1 + length_out - *(sub_FP_3+ (f_row & 0x1)) );
						DMA2DA = (TEMP_Ptr  +  f_row * length_out + length_out - *(sub_FP_3+ (f_row & 0x1)) );
						DMA2SZ = fp_temp ;
			...
			if((LAYER->FOOTPRINT & 0x0001)){
				...
				if(PARA->KERNEL_H == 1 ){
					...
					for(; f_row < ks_h ; f_row++ ){
						fp_temp = sub_FP_3[(f_row & 0x0001)];
						//reset FP for next sub-op
						*(sub_FP_3+ ((f_row+1) & 0x0001)) =length_out;

						if(fp_temp){
							int buf_idx = (f_row + ks_h - 1) % (ks_h+1);
							int _buf_idx = (f_row + ks_h - 2) % (ks_h+1);
							DMACTL0 = DMA0TSEL__DMAREQ;
							DMA0CTL = DMADT_1 + DMADSTINCR_3 + DMASRCINCR_3 ;
							if(f_row==1){
								DMA0SA = (TEMP_Ptr + (length_out) - fp_temp );
							}else{
								DMA0SA = (TEMP_Ptr + (length_out * ( _buf_idx + 1 )) - fp_temp );
							}
							...
							DMA0SA = (TEMP_Ptr + (f_row+1) * length_out - fp_temp );
							...
							if(f_row == PARA->KERNEL_H-1){
								DMA2DA = ((TEMP_Ptr + (PARA->KERNEL_H+in_ch+1) * length_out) - fp_temp );
							}
							DMA2SZ = fp_temp ;
							DMA2CTL |= DMAEN__ENABLE;
				...
				for( ; in_ch < D_IN->CH ; in_ch++ ){
					...
					*(sub_FP_3+ ((in_ch+1) & 0x0001)) = length_out;
					if(fp_temp){
						...
						int _buf_idx = (in_ch + D_IN->CH - 2) % (D_IN->CH+1);
						if(in_ch==1)
							DMA0SA = (TEMP_Ptr + ((PARA->KERNEL_H+1) * length_out) - fp_temp);
						else
							DMA0SA = (TEMP_Ptr + ((PARA->KERNEL_H+_buf_idx+1) * length_out) - fp_temp);
						DMA0DA = LEA_MEMORY;
						DMA0SZ = fp_temp;
						if(DMA0SZ){
						DMA0CTL |= DMAEN__ENABLE + DMAREQ + DMAIE;
						...
						DMA0SA = (TEMP_Ptr + (PARA->KERNEL_H+in_ch+1) * length_out - fp_temp );
						DMA0DA = (LEA_MEMORY + (length_out<<1) - fp_temp );
						DMA0CTL |= DMAEN__ENABLE + DMAREQ+DMAIE;
						__bis_SR_register(GIE+LPM0_bits);
						...
						DMA2CTL = DMADT_0 + DMADSTINCR_3 + DMASRCINCR_3 + DMALEVEL__LEVEL;
						DMA2SA = (LEA_MEMORY + length_out - fp_temp);
						DMA2DA = (TEMP_Ptr + (PARA->KERNEL_H+buf_idx+1) * length_out - fp_temp);
						DMA2SZ = fp_temp ;
						DMA2CTL |= DMAEN__ENABLE;
		...
		if(in_ch == D_IN->CH +1){
			...
			if(fp_temp){
				...
				*(LEA_MEMORY + length_out) = *(LEA_MEMORY + length_out + 1) = *(BAIS_Ptr+out_ch);
				...
				LEAPMS0 = MSP_LEA_CONVERT_ADDRESS(LEA_MEMORY);
		...
		if(in_ch == D_IN->CH +2){
			...
			if(*sub_FP < D_OUT->W * D_OUT->H){
				...
				DMA0CTL = DMADT_1 + DMADSTINCR_3 + DMASRCINCR_3 ;
				DMA0SA = (TEMP_Ptr + PARA->KERNEL_H * D_IN->W * D_OUT->H + h * D_IN->W + w);
			...
			for(;h<D_OUT->H;h++){
				for(;w<D_OUT->W;w++){
					...
					tmp = tmp < _Q15(-0.25)? _Q15(-0.25) : tmp;
					*(D_OUT_Ptr +
							out_ch * (D_OUT->H * D_OUT->W) +
							h *  D_OUT->W + w)
					= tmp;// < 0 ? 0 : *(LEA_BUFFER_Ptr+ h *  D_IN->W + w);
					*sub_FP++;
				}
				w=0;
			}
